...
import { InlineComment, LLMClient, LLMConfig, ReviewResult } from '../interfaces/llm-client.interface';
...
export abstract class BaseLLMClient implements LLMClient {
  ...
  abstract generateReview(diff: string, commitMessages: string): Promise<ReviewResult>;
  ...
  protected parseReviewResult(aiOutput: string): ReviewResult {
    try {
      ...
      return { overall, inlineComments };
    } catch (error) {
      console.error('解析 AI 审查结果失败:', error.message);
      // 返回默认空结构，避免程序中断
      return { overall: '', inlineComments: [] };
    }
  }
